 ...
 *
 * calculateThermalResistance(): Calculates the motor power factor
 * calculateCurrent():
 * calculateEfficiency():
 * calculateEfficiency():
 ...
 * Used to calculateThermalResistance the power supplied by the motor to the pump shaft.
 */
  ...
  public:
    struct Output {
        ...
        Output(const double shaftPower, const double current, const double powerFactor, const double efficiency,
            ...
            : shaftPower(shaftPower), current(current), powerFactor(powerFactor), efficiency(efficiency), power(power),
              estimatedFLA(estimatedFLA), loadFactor(loadFactor) {}
     ...
     *
     */
    MotorShaftPower(const double motorRatedPower, const double motorMeasuredPower, const double motorRPM,
                    const Motor::LineFrequency lineFrequency, const Motor::EfficiencyClass efficiencyClass,
                    const double specifiedEfficiency, const double ratedVoltage, const double fullLoadAmps,
                    const double fieldVoltage, const Motor::LoadEstimationMethod loadEstimationMethod,
                    const double fieldCurrent)
        : motorRatedPower(motorRatedPower), fieldPower(motorMeasuredPower), motorRPM(motorRPM),
          lineFrequency(lineFrequency), ratedVoltage(ratedVoltage), fullLoadAmps(fullLoadAmps),
          fieldVoltage(fieldVoltage), efficiencyClass(efficiencyClass), specifiedEfficiency(specifiedEfficiency),
          fieldCurrent(fieldCurrent), loadEstimationMethod(loadEstimationMethod) {};

    /**
     * Calculate motor shaft power
     * @return MotorShaftPower::Output, class containing all the results of the MotorShaftPower calculations, see Output
