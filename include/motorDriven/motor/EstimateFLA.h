...
class EstimateFLA {
  public:
     ...
     */
    EstimateFLA(double motorRatedPower, double motorRPM, Motor::LineFrequency lineFrequency,
        ...
        : motorRatedPower(motorRatedPower), motorRPM(motorRPM), lineFrequency(lineFrequency),
          efficiencyClass(efficiencyClass), specifiedEfficiency(specifiedEfficiency), ratedVoltage(ratedVoltage) {
        calculate();
    }
     ...
     */
    double getEstimatedFLA() const { return estimatedFLA; }
  ...
  private:
    ...
    std::array<std::array<double, 3>, 5> peFlaMultipliers = {{{{0.972803032, 0.988822468, 0.990453682}},
