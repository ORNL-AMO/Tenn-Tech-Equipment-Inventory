 ...
 * Used to calculateReynoldsNumber velocity and velocity head so those values can be used in the HeadToolSuctionTank
 * class or HeadTool class to calculateThermalResistance all of the values in the returned map.
 */
class HeadToolBase {
  public:
    struct Output {
        Output(double elevationHead, double pressureHead, double velocityHeadDifferential, double suctionHead,
               double dischargeHead, double pumpHead)
            : elevationHead(elevationHead), pressureHead(pressureHead),
              velocityHeadDifferential(velocityHeadDifferential), suctionHead(suctionHead),
              dischargeHead(dischargeHead), pumpHead(pumpHead) {}

        Output()             = default;
        double elevationHead = 0, pressureHead = 0, velocityHeadDifferential = 0, suctionHead = 0, dischargeHead = 0,
               pumpHead = 0;
    };
    /**
    ...
    HeadToolBase(const double specificGravity, const double flowRate, const double suctionPipeDiameter,
                 const double suctionLineLossCoefficients, const double dischargePipeDiameter,
                 const double dischargeGaugePressure, const double dischargeGaugeElevation,
                 const double dischargeLineLossCoefficients)
        : specificGravity(specificGravity), flowRate(flowRate), suctionPipeDiameter(suctionPipeDiameter),
          suctionLineLossCoefficients(suctionLineLossCoefficients), dischargePipeDiameter(dischargePipeDiameter),
          dischargeGaugePressure(dischargeGaugePressure), dischargeGaugeElevation(dischargeGaugeElevation),
 ...
 * Used to calculateThermalResistance all of the values in the returned map.
 */
class HeadToolSuctionTank : private HeadToolBase {
  public:
     ...
     * @param suctionTankGasOverPressure double, gas over pressure of suction tank in psig (pounds per square inch
     * gauage)
    ...
    HeadToolSuctionTank(const double specificGravity, const double flowRate, const double suctionPipeDiameter,
                        const double suctionTankGasOverPressure, const double suctionTankFluidSurfaceElevation,
                        const double suctionLineLossCoefficients, const double dischargePipeDiameter,
                        const double dischargeGaugePressure, const double dischargeGaugeElevation,
                        const double dischargeLineLossCoefficients)
        : HeadToolBase(specificGravity, flowRate, suctionPipeDiameter, suctionLineLossCoefficients,
                       dischargePipeDiameter, dischargeGaugePressure, dischargeGaugeElevation,
 ...
 * Used to calculateThermalResistance all of the values of the returned unordered map.
 */
class HeadTool : private HeadToolBase {
  public:
    ...
    HeadTool(const double specificGravity, const double flowRate, const double suctionPipeDiameter,
             const double suctionGaugePressure, const double suctionGaugeElevation,
             const double suctionLineLossCoefficients, const double dischargePipeDiameter,
             const double dischargeGaugePressure, const double dischargeGaugeElevation,
             const double dischargeLineLossCoefficients)
        : HeadToolBase(specificGravity, flowRate, suctionPipeDiameter, suctionLineLossCoefficients,
                       dischargePipeDiameter, dischargeGaugePressure, dischargeGaugeElevation,
                       dischargeLineLossCoefficients),
          suctionGaugePressure(suctionGaugePressure), suctionGaugeElevation(suctionGaugeElevation) {}
    /**
     * Calculates the operating pump head
    ...
    HeadToolBase::Output calculate() override;
    // std::unordered_map<std::string, double> calculateThermalResistance() override;
