...
class MoverEfficiency {
  ...
  public:
     ...
     */
    MoverEfficiency(double const specificGravity, double const flowRate, double const head,
                    double const moverShaftPower)
        : specificGravity(specificGravity), head(head), flowRate(flowRate), moverShaftPower(moverShaftPower),
          isPump(true) {}
     ...
     */
    MoverEfficiency(double const flowRate, double const fanShaftPower, double const inletPressure,
                    double const outletPressure, double const compressibilityFactor, double const velocityPressure)
        : flowRate(flowRate), moverShaftPower(fanShaftPower), inletPressure(inletPressure),
          outletPressure(outletPressure), compressibilityFactor(compressibilityFactor), isPump(false),
  ...
  public:
     ...
     */
    double getMoverShaftPower() const { return moverShaftPower; }

    /**
     * Set the pump shaft power
     * @param moverShaftPower double, pump shaft power in hp
     */
    void setMoverShaftPower(double moverShaftPower) { this->moverShaftPower = moverShaftPower; }
