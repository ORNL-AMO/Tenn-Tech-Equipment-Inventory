...
class ResultData {
  public:
    ResultData(const double flow, const double pressure, const double power, const double efficiency)
        : flow(flow), pressure(pressure), power(power), efficiency(efficiency) {};
  ...
  public:
    ...
    struct BaseCurve {
        ...
        BaseCurve(const double flow, const double pressure, const double power)
            : flow(flow), pressure(pressure), power(power) {};
    ...
    struct BaseOperatingPoint : public RatedPoint {
        ...
        BaseOperatingPoint(const double flow, const double pressure, const double power, const double density,
            ...
            : RatedPoint(flow, pressure, power, density, speed, speedCorrected), pressureBarometric(pressureBarometric),
              usePt1Factor(usePt1Factor), pt1(pt1) {};
    ...
    FanCurveData(FanCurveType const curveType, std::vector<BaseCurve> baseCurveData)
        : curveType(curveType), baseCurveData(std::move(baseCurveData)), calcType(CalculationType::BaseCurve) {}

    FanCurveData(FanCurveType const curveType, std::vector<RatedPoint> ratedPointData)
        : curveType(curveType), ratedPointData(std::move(ratedPointData)), calcType(CalculationType::RatedPoint) {}

    FanCurveData(FanCurveType const curveType, std::vector<BaseOperatingPoint> baseOperatingPointData)
        : curveType(curveType), baseOperatingPointData(std::move(baseOperatingPointData)),
...
class FanCurve {
  public:
    FanCurve(const double density, const double densityCorrected, const double speed, const double speedCorrected,
        ...
        : density(density), densityCorrected(densityCorrected), speed(speed), speedCorrected(speedCorrected),
          pressureBarometric(pressureBarometric), pressureBarometricCorrected(pressureBarometricCorrected),
          pt1Factor(pt1Factor), gamma(gamma), gammaCorrected(gammaCorrected), area1(area1), area2(area2),
