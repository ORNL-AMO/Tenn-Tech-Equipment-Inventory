...
class VelocityPressureTraverseData {
  public:
    double getPv3Value() const { return pv3; }

    double get75percentRule() const { return percent75Rule; }
...
class FlangePlane : public Planar {
  public:
    FlangePlane(const double area, const double tdx, const double pbx) : Planar(area, tdx, pbx, 0) {}
};
...
class TraversePlane : public Planar, public VelocityPressureTraverseData {
  public:
    TraversePlane(const double area, const double tdx, const double pbx, const double psx,
        ...
        : Planar(area, tdx, pbx, psx), VelocityPressureTraverseData(pitotTubeCoefficient, std::move(traverseHoleData)) {
    }
};
