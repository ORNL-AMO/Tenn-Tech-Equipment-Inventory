...
void CHP::calculate() {
    ...
    auto const purchasedElectricity =
        Cases(annualElectricityConsumption, annualElectricityConsumption - generatedElectricity.chpCase);
    auto const chpThermal = Cases(0, (generatedElectricity.chpCase * chpThermalOutput * thermalUtilization) / 1000000);
    auto const onSiteBoiler =
        Cases(annualThermalDemand,
              (annualThermalDemand - chpThermal.chpCase < 0) ? 0 : annualThermalDemand - chpThermal.chpCase);
    auto const boilerHeaterFuel =
        Cases(annualThermalDemand / displacedThermalEfficiency, onSiteBoiler.chpCase / displacedThermalEfficiency);
    ...
    // cost in $ stuff
    auto const cost = (annualElectricityConsumption * avgElectricityCosts) -
                      (generatedElectricity.chpCase * avgElectricityCosts * percentAvgkWhElectricCostAvoided);

    auto const purchasedElectricityDollars =
        Cases(annualElectricityConsumption * avgElectricityCosts,
    ...
    auto const totalOperatingCosts =
        Cases(purchasedElectricityDollars.baseCase + onSiteBoilerFuelDollars.baseCase,
              purchasedElectricityDollars.chpCase + standbyCharges.chpCase + chpFuelDollars.chpCase +
    ...
    auto const thermalCredit =
        -(onSiteBoilerFuelDollars.baseCase - onSiteBoilerFuelDollars.chpCase) / generatedElectricity.chpCase;
    ...
    // CHP::CostInfoOutput {annualOperationSavings, totalInstalledCostsPayback, simplePayback, fuelCosts, thermalCredit,
    // incrementalOandMDollarsKwH, totalOperatingCostsToGenerate};
    this->costInfo = {annualOperationSavings,     totalInstalledCostsPayback,   simplePayback, fuelCosts, thermalCredit,
                      incrementalOandMDollarsKwH, totalOperatingCostsToGenerate};
}
...
void CHP::setStandbyRate(const double standbyRate) { this->standbyRate = standbyRate; }
...
void CHP::setChpThermalOutput(const double chpThermalOutput) { this->chpThermalOutput = chpThermalOutput; }

void CHP::setChpAvailability(const double chpAvailability) { this->chpAvailability = chpAvailability; }

void CHP::setThermalUtilization(const double thermalUtilization) { this->thermalUtilization = thermalUtilization; }

void CHP::setAvgPowerDemand(const double avgPowerDemand) { this->avgPowerDemand = avgPowerDemand; }

void CHP::setAvgThermalDemand(const double avgThermalDemand) { this->avgThermalDemand = avgThermalDemand; }

void CHP::setNetCHPpower(const double netCHPpower) { this->netCHPpower = netCHPpower; }
