...
double OptimalSpecificSpeedCorrection::calculate() {
    const double notVTP[7][2] = {{14.884085, 6.5703317},          {-0.021342141, -0.010048327},
                                 {0.0000121172, 0.0000061475098}, {-0.0000000031885957, -0.0000000019659725},
                                 {1.4907927E-13, 3.5079978E-13},  {1.2855352E-16, -3.2272033E-17},
    ...
    if (style == Pump::Style::VERTICAL_TURBINE) {
        ...
        return std::fmax((VTP[0][index] + (VTP[1][index] * specificSpeed) + (VTP[2][index] * pow(specificSpeed, 2)) +
                          (VTP[3][index] * pow(specificSpeed, 3)) + (VTP[4][index] * pow(specificSpeed, 4)) +
                          (VTP[5][index] * pow(specificSpeed, 5)) + (VTP[6][index] * pow(specificSpeed, 6))),
                         0) /
               100;
    }
    ...
    return std::fmax((notVTP[0][index] + (notVTP[1][index] * specificSpeed) +
                      (notVTP[2][index] * pow(specificSpeed, 2)) + (notVTP[3][index] * pow(specificSpeed, 3)) +
                      (notVTP[4][index] * pow(specificSpeed, 4)) + (notVTP[5][index] * pow(specificSpeed, 5)) +
                      (notVTP[6][index] * pow(specificSpeed, 6))),
                     0) /
           100;
}
