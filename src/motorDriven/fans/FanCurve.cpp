...
std::vector<ResultData> FanCurve::calculateBaseCurve() {
    ...
    for (auto const& row : this->curveData.baseCurveData) {
        ...
        for (auto i = 0; i < 7; i++) {
            ...
            // eq 18, 19 and 20 depend on the type of FanCurve, here pressure (row.pressure) represents the pressure
            // type?
            ...
            auto const zc   = ((this->gammaCorrected - 1) / this->gammaCorrected) * (6362 * hC / qC) /
                            (pt1c + 13.63 * this->pressureBarometricCorrected);          // eq 30
...
std::vector<ResultData> FanCurve::calculateRatedPoint() {
    ...
    for (auto const& row : this->curveData.ratedPointData) {
        ...
        for (auto i = 0; i < 7; i++) {
            ...
            estPtc = estPt * std::pow(row.speedCorrected / row.speed, 2) * (this->densityCorrected / row.density) *
                     (kp / kpC);
