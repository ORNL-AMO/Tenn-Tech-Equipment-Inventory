 ...
 *      calculateThermalResistance(): Calculates the estimated FLA and 25% interval values of Current.
 *
...
#include "motorDriven/motor/Poles.h"

/**
 * Estimating FLA methodology
 * - Pumps has four different methods of estimating FLA, with the four methods corresponding to the four efficiency
 * choices.
 *  - For Standard Efficiency selection, it selects the FLA for the motor size and speed class that has been selected.
 * For example, the SE FLA for a 200 hp 4-pole motor is 228.6 amps.
 *  - For Energy Efficient selection, it selects the FLA for the motor size and speed class that has been selected.  For
 * example, the EE FLA for a 200 hp 4-pole motor is 225.8 amps.
 *  - For Average Efficiency selection, it averages the FLA for Energy Efficient and Standard Efficiency choices.  The
 * average of the two values listed above is 227.2 amps, which is what Pumps returns.
 *  - For a case where the efficiency has been specified, there is a multiple step process:
 *      1. The absolute value of the differences between the specified efficiency and the efficiency values for EE and
 * SE motors are calculated
 *      2. Pick the smaller of the two absolute values, and use the nominal efficiency of that selection going forward
 *      3. Divide the selected (EE or SE) efficiency by the specified efficiency and then multiply that by the FLA for
 * the corresponding selection.  See the example below.
 *
 ...
 * Use an example of a motor known to have a 575 volt rating, but the FLA is not readable and the user wants to Estimate
 * the FLA The process is the same as above, except that the estimated FLA is adjusted in inverse proportion to the
 * rated voltage.
 *
...
const std::array<std::array<double, 6>, 5> EstimateFLA::calculatePartialLoadCoefficients(const int pole) const {
    if (pole == 0) {
        return {{{{0.2385152, 0.31999999, 0.51999998, 0.75400001, 1, 1.253}},
        ...
        return {{{{0.26803651, 0.3535319, 0.54000002, 0.76428652, 1, 1.251}},
        ...
        return {{{{0.28999999, 0.39519981, 0.55500001, 0.76973772, 1, 1.254}},
        ...
        return {{{{0.33000001, 0.43000001, 0.56062472, 0.78429687, 1, 1.237}},
        ...
        return {{{{0.37, 0.44, 0.57857579, 0.78581947, 1, 1.233}},
    ...
    // pole == 5 here
    return {{{{0.41999999, 0.5, 0.61000001, 0.80000001, 1, 1.215}},
             {{0.04507853, 0.04205693, 0.06270286, 0.01749944, 0, 0}},
             {{0.00098519, 0.00121622, 0.0005819, 0.00068763, 0, 0}},
             {{0.20683549, 0.1764105, 0.08895831, 0.01809594, 0, 0}},
             {{0.0551472, 0.05516704, 0.05197735, 0.03981217, 0, 0}}}};
...
std::array<double, 6> EstimateFLA::calculate() {
     ...
     */
    double const basicFLAValue =
        motorRatedPower * (flaBasic[0][pole] + (flaBasic[1][pole] * std::exp(-flaBasic[2][pole] * motorRatedPower)) +
    ...
    auto const plMult = [&partialLoadCoefficients, this](std::size_t i) {
        return partialLoadCoefficients[0][i] +
               (partialLoadCoefficients[1][i] * std::exp(-1 * partialLoadCoefficients[2][i] * motorRatedPower)) +
               (partialLoadCoefficients[3][i] * std::exp(-1 * partialLoadCoefficients[4][i] * motorRatedPower));
    };

    const std::array<double, 6> plMultiplier = {{plMult(0), plMult(1), plMult(2), plMult(3), plMult(4), plMult(5)}};

    // used to calculateThermalResistance FLA
    ...
    if (efficiencyClass == Motor::EfficiencyClass::PREMIUM) {
        double const peMultiplier =
            peFlaMultipliers[0][pole] +
        ...
        estimatedFLA            = adjustForVoltage(peFLAValue * plMultiplier[4], 0);
        return {{peFLAValue * plMultiplier[0], peFLAValue * plMultiplier[1], peFLAValue * plMultiplier[2],
                 peFLAValue * plMultiplier[3], peFLAValue * plMultiplier[4], peFLAValue * plMultiplier[5]}};

        //        auto motorEfficiency = MotorEfficiency(lineFrequency, motorRPM,
        //        Motor::EfficiencyClass::ENERGY_EFFICIENT, motorRatedPower); const double effValEE =
        //        motorEfficiency.calculateThermalResistance(1);
        //        motorEfficiency.setEfficiencyClass(Motor::EfficiencyClass::PREMIUM);
        //        const double effValPE = motorEfficiency.calculateThermalResistance(1);
    ...
    if (efficiencyClass == Motor::EfficiencyClass::ENERGY_EFFICIENT) {
        estimatedFLA = adjustForVoltage(eeFLAValue * plMultiplier[4], 0);
        return {{eeFLAValue * plMultiplier[0], eeFLAValue * plMultiplier[1], eeFLAValue * plMultiplier[2],
        ...
        estimatedFLA = adjustForVoltage(seFLAValue * plMultiplier[4], 0);
        return {{seFLAValue * plMultiplier[0], seFLAValue * plMultiplier[1], seFLAValue * plMultiplier[2],
        ...
        /**
         * For a case where the efficiency has been specified, there is a multiple step process:
         * 1. The absolute value of the differences between the specified efficiency and the efficiency values for EE
         * and SE motors are calculated
         * 2. Pick the smaller of the two absolute values, and use the nominal efficiency of that selection going
         * forward
         * 3. Divide the selected (EE or SE) efficiency by the specified efficiency and then multiply that by the FLA
         * for the corresponding selection.
         */

        auto const seeffVal =
            MotorEfficiency(lineFrequency, motorRPM, Motor::EfficiencyClass::STANDARD, motorRatedPower)
                .calculate(1, specifiedEfficiency);

        auto const eeeffVal =
            MotorEfficiency(lineFrequency, motorRPM, Motor::EfficiencyClass::ENERGY_EFFICIENT, motorRatedPower)
                .calculate(1, specifiedEfficiency);
