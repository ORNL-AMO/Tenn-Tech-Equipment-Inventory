...
MotorShaftPower::Output MotorShaftPower::calculate() {
    ...
    if (loadEstimationMethod == Motor::LoadEstimationMethod::POWER) {
        ...
        const double adjpf1 =
            pf1 / ((((fieldVoltage / ratedVoltage - 1) * (-2 * lf1 + 1)) + 1) * (fieldVoltage / ratedVoltage));
        const double adjpf2 =
            pf2 / ((((fieldVoltage / ratedVoltage - 1) * (-2 * lf2 + 1)) + 1) * (fieldVoltage / ratedVoltage));
        ...
        MotorEfficiency motorEfficiency(lineFrequency, motorRPM, efficiencyClass, motorRatedPower);
        efficiency  = motorEfficiency.calculate(tempLoadFraction, specifiedEfficiency);
        powerFactor = MotorPowerFactor(lineFrequency, motorRPM, efficiencyClass, specifiedEfficiency, motorRatedPower,
                                       tempLoadFraction, current, efficiency, ratedVoltage)
                          .calculate();
        MotorPower motorPower(ratedVoltage, current, powerFactor);
        power    = motorPower.calculate();
        current1 = current;
