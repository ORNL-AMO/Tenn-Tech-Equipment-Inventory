...
HeadToolBase::Output HeadToolSuctionTank::calculate() {
    ...
    //			(((dischargeGaugePressure - suctionTankGasOverPressure) * 0.145037738007) / 1.42197020632) /
    //specificGravity;
    ...
    const double pressureHead = ((dischargeGaugePressure - suctionTankGasOverPressure) / 0.432750001) / specificGravity;
...
HeadToolBase::Output HeadTool::calculate() {
    ...
    const double pressureHead  = ((dischargeGaugePressure - suctionGaugePressure) / 0.432750001) / specificGravity;
    const double velocityHeadSuction   = velocityHead(velocity(suctionPipeDiameter / 12, flow), gravity);
    const double velocityHeadDischarge = velocityHead(velocity(dischargePipeDiameter / 12, flow), gravity);
